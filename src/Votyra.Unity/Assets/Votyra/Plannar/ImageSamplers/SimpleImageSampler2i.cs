...
namespace Votyra.Plannar.ImageSamplers
{
    public class SimpleImageSampler2i : IImageSampler2i
    {
        ...
        {
            return image.Sample (new Vector2i (pos.x, pos.y)).FloorToInt ();
        }

        public int SampleX0Y1 (IImage2f image, Vector2i pos)
        {
            return image.Sample (new Vector2i (pos.x, pos.y + 1)).FloorToInt ();
        }

        public int SampleX1Y0 (IImage2f image, Vector2i pos)
        {
            return image.Sample (new Vector2i (pos.x + 1, pos.y)).FloorToInt ();
        }

        public int SampleX1Y1 (IImage2f image, Vector2i pos)
        {
            return image.Sample (new Vector2i (pos.x + 1, pos.y + 1)).FloorToInt ();
        }
    }
}